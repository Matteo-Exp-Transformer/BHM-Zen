...
| FU-004 | 06-07-26 | `aree/TESTING_SKILL.md` compilata: Vitest + `verify:flows`/`verify:flows:write` + QA manuale HEALTH_CHECK §5 (Playwright resta roadmap) | Report-fondamenta-fable | fatto 06-07-26 |
...
| FU-008 | 06-07-26 | Applicare migration `20260706070000` (trigger storno-aware manutenzioni) e abilitare lo storno manutenzioni in UI — **fatto CP9** (ok owner, dry-run pulito, history 11/11; `useStorna` unificata mansioni+manutenzioni). Nota: l'header «DRAFT» dentro la migration è superato ma il file è LOCK (applicata) e non si tocca | Report-port-fu002-fable | fatto 06-07-26 |
| FU-009 | 06-07-26 | E2E in SCRITTURA con utente test — **fatto CP9** (ok owner): `npm run verify:flows:write` (flag `--write`, il run base resta sola-lettura): registra → trigger avanza next_due → storno manutenzione (trigger storno-aware verificato live) → spunta+storno mansione → timbro + append-only su shift_seals | Report-port-fu002-fable | fatto 06-07-26 |
